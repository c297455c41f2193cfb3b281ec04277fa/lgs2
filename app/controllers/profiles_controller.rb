class ProfilesController < ApplicationController
  ...
  # GET /profiles/1.json
  def show
      #if current_user.id == Profile.find(params[:id].to_i).user_id || current_user.try(:admin?)
      #  @profile = Profile.find_by_user_id(current_user.id)
      #else 
      #  # user can only see its own data
      #  @profile = Profile.find_by_user_id(current_user.id)
